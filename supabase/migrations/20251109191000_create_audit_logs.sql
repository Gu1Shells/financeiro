...
BEGIN
  IF TG_OP = 'DELETE' THEN
    action_description := 'Despesa "' || OLD.description || '" excluída - Valor: R$ ' ||
                         (OLD.total_value / 100.0)::numeric(10,2);
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    action_description := 'Despesa "' || NEW.description || '" atualizada';
  ...
  ELSIF TG_OP = 'INSERT' THEN
    action_description := 'Despesa "' || NEW.description || '" criada - Valor: R$ ' ||
                         (NEW.total_value / 100.0)::numeric(10,2);
...
DECLARE
  ...
  action_description text;
BEGIN
  SELECT description INTO expense_desc FROM expenses WHERE id = NEW.expense_id;

  IF TG_OP = 'UPDATE' AND OLD.paid = false AND NEW.paid = true THEN
    action_description := 'Parcela ' || NEW.installment_number || ' de "' || expense_desc ||
                         '" paga - Valor: R$ ' || (NEW.value / 100.0)::numeric(10,2);
    INSERT INTO audit_logs (user_id, action, entity_type, entity_id, description, old_values, new_values)
    VALUES (
      auth.uid(),
      'update',
...
DROP TRIGGER IF EXISTS installment_payment_audit_trigger ON installments;
